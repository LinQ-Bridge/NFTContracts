...
contract LinQNFTWrapper is Ownable, Pausable, ReentrancyGuard {
    ...
    event LinQWrapperLock(address indexed fromAsset, address indexed sender, uint64 toChainId, address toAddress, uint256 tokenId, address feeToken, uint256 fee);
    event LinQWrapperSpeedUp(address indexed feeToken, bytes indexed txHash, address indexed sender, uint256 efee);
    ...
    function setFeeCollector(address collector) external onlyOwner {
        require(collector != address(0), "empty address");
    ...
    function setLockProxy(address _lockProxy) external onlyOwner {
        ...
        require(ILinQNFTLockProxy(lockProxy).managerProxyContract() != address(0), "not lock proxy");
    }
    ...
    function lock(address fromAsset, uint64 toChainId, address toAddress, uint256 tokenId, address feeToken, uint256 fee) external payable nonReentrant whenNotPaused {
        require(toChainId != chainId && toChainId != 0, "!toChainId");

        _pull(feeToken, fee);
        _push(fromAsset, toChainId, toAddress, tokenId);
        emit LinQWrapperLock(fromAsset, msg.sender, toChainId, toAddress, tokenId, feeToken, fee);
    }

    function speedUp(address feeToken, bytes calldata txHash, uint256 fee) external payable nonReentrant whenNotPaused {
        _pull(feeToken, fee);
        emit LinQWrapperSpeedUp(feeToken, txHash, msg.sender, fee);
    }
